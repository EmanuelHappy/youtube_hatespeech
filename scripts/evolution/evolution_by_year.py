...
c = 0
not_comm = 0
with open("authors_evolution.pickle", "rb") as fp:
...
for value in authors:
    ...
    for comm in value:
        ...
        if cat == "Alt-right" or cat == "Alt-lite" or cat == "Intellectual Dark Web":
            ...
            d[comm_transl["control"]].append((comm["timestamp"], comm["id"]))
            diff_comm.add(comm_transl["control"])
    ...
    for cat in diff_comm:
        ...
        if d[cat][0][0] < bins_y_s[0][0]:
            ...
            year = bins_t_s[2]
        if year != "year":
            for i in range(1, min(11, len(d[cat])+1)):
...
for cat in communities:
    for year in bins_t_s:
        for i in range(1, 11):
            ...
            with open(f"./data/years/evolution_{cat}_{year}_{i}.pickle", "wb") as fp:
