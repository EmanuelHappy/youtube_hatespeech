...
for year in bins_t_s:
    for name in names:
        ...
        for num in range(2, 11, 2):
            ...
            with open(f"./data/evolution10_{name}_{year}_{num}.pickle", "rb") as fp:
                evolution_id = pickle.load(fp)

            print(name, year, "comment:", num, "number of comments:", len(evolution_id))
            if name == "Alt-lite":
                evolution_id = list(evolution_id)
                evolution_id.extend(to_change[num])
                print("new number =", len(evolution_id))

            for ide in evolution_id:
                if ide in ks and len(ks[ide]) > 0:
                    if name != "Alt-lite" and ide in changed:
                        to_change[num].append(ide)
            ...
            print(f"Shape of values = {values.shape}, Changed = {len(to_change[num])}, Removed = {t_remove}")
            t_remove = 0
        ...
        df = pd.DataFrame(d)
        df.to_csv(f"{dst_path}{name}_{year}_perspective_evolution10.csv")
