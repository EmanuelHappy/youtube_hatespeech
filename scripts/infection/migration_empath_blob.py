...
old_emotion_list = ['size', 'sadness', 'independence', 'positive_emotion', 'family',
...
for name in names:
    for start in bins_t_s:
        for cat in ["light", "mild", "severe"]:
            ...
            for year in range(int(start) - 1, int(start) + 1):
                ...
                for ide in migration_id:
                    if ide in ks_emp and ide in ks_pol:
                        for emotion in emotion_list:
                            if ks_emp[ide][old_emotion_list.index(emotion)] > 0:
            ...
            d["year"] = x
            print("saving", d["hate"], d["hate_dyd"], d["hate_dyu"], x)

            df = pd.DataFrame(d)
            df.to_csv(f"{dst_path}{name}{start}{cat}_empath_blob_migration_new.csv")
